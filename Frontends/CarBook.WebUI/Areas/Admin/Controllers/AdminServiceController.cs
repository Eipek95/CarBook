...
namespace CarBook.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminServiceController : Controller
    {
        ...
        public async Task<IActionResult> CreateService(CreateServiceDto serviceDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7177/api/Service", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7177/api/Service/?id={id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"https://localhost:7177/api/Service/{id}");
        ...
        public async Task<IActionResult> UpdateService(UpdateServiceDto serviceDto)
        {
            ...
            var responseMessage = await client.PutAsync("https://localhost:7177/api/Service/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
